...
def save_filtered_sqanti_gtf(gtf_file, filtered_isoforms, results):
    ...
    with open(gtf_file, "r") as ifile, open(f"{results.out_dir}/filtered_{base_name}", "w") as ofile:
...
def save_filtered_sqanti_fasta(fasta_file, filtered_isoforms, results):
    ...
    SeqIO.write(filtered_sequences, f"{results.out_dir}/filtered_{base_name}", "fasta")
...
def main():
    ...
    classification.to_csv(f"{results.out_dir}/filtered_{base_name}.tsv", index=False, sep = "\t")
    save_filtered_sqanti_gtf(results.corrected_gtf,filtered_isoforms, results)
    save_filtered_sqanti_fasta(results.corrected_fasta, filtered_isoforms, results)
