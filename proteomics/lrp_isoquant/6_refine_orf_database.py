...
def aggregate_results(pacbio, orfs):
    ...
    fl_dict = pd.Series(orfs.FL_count.values,index=orfs.pb_acc).to_dict()
    cpm_dict = pd.Series(orfs.CPM.values,index=orfs.pb_acc).to_dict()
    
    pacbio['FL_count'] = pacbio['accessions'].apply(lambda accs : get_total(accs, fl_dict))
...
def main():
    ...
    pacbio = pacbio[['pb_accs', 'base_acc', 'coding_score','orf_calling_confidence','upstream_atgs','orf_score','gene','FL_count', 'CPM']]
