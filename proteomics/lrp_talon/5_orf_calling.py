...
def main():
    ...
    ############################################# modified #############################################
    classification = classification[['annot_transcript_id', 'FL']]
    ############################################# modified #############################################
    total = classification['FL'].sum()
    classification['CPM'] = classification['FL'] / total * 1000000

    ############################################# modified #############################################
    orfs = pd.merge(orfs, pb_gene, left_on = 'pb_acc', right_on = 'transcript_id', how = 'left')

    orfs = pd.merge(orfs, classification, left_on = 'pb_acc', right_on = 'annot_transcript_id', how = 'left')
    orfs = orfs.drop(columns = ['annot_transcript_id'])
    ############################################# modified #############################################
